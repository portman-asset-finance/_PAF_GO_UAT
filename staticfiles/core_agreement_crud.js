...
$(function() {
    ...
    var companyAutoComplete = $("#company_name");

    companyAutoComplete.focusout(function() {
        ...
        if (! $("#customer-switch").is(":checked")) {
            if ( (! customer_number && ! $("#customer-number-display").html()) && $("#company_name").val()) {
    ...
    try {
        companyAutoComplete.easyAutocomplete({
            adjustWidth: false,
            categories: [{
                'listLocation': 'data',
            ...
            url: function (p) {
                return '/core_agreement_crud/customer/search?search_value=' + p;
            },
            list: {
                onSelectItemEvent : function() {
                    $("#company_name").removeClass("error");
                    company_name_err.html("").hide() ;
                    //console.log(companyAutoComplete.getSelectedItemData());
                },
                onChooseEvent: function () {
                    console.log('chosen');
                    var match = /\((\w+)\)$/.exec(companyAutoComplete.getSelectedItemData());
                    if (match) {
                        getCustomer(match[1]);
                        // $("#id-customer-number").attr("disabled", false).val(match[1]).attr("readonly", true);
                    }
                }
            }
        });
        ...
        console.log(err);
        // Sometimes this library will not be included as it's not needed on certain pages.
    }

    companyAutoComplete.focusout(function() {
        var match = /\((\w+)\)$/.exec(companyAutoComplete.getSelectedItemData());
        if(match) getCustomer(match[1]) ;
    });

    $("#confirm-changes").on("hidden.bs.modal", function() {
        $(":submit").removeAttr("disabled");
    });

    $(".agreement-list").click(function(e) {
        ...
        if(typeof(Storage) !== "undefined") {
            if(sessionStorage.agreementListReferer && sessionStorage.agreementListReferer !== 'false') {
                url = sessionStorage.agreementListReferer;
            } else {
                url = '/core_agreement_crud/agreementlist';
            }
        }
    ...
    */
    ...
    $("#customer-switch").click(function() {
        ...
        if($(this).is(":checked")) {
            ...
            $("#id-company-name-label").html("New Company Name");
            ...
            $("#id-company-name-label").html("Company Name Search");
...
function checkCustomerChanges() {
    ...
    if(company_name && company_name !== customer.customercompany && customer.customercompany !== null) {
        changes++;
    ...
    if(address1 && address1 !== customer.customeraddress1 && customer.customeraddress1 !== null) {
        changes++;
        changes_html += "<li>Address line 1 from '" + customer.customeraddress1 + "' to '" + address1 + "'</li>" ;
    }

    var address2 = $("#id-address-line-2").val();
    if(address2 && address2 !== customer.customeraddress2 && customer.customeraddress2 !== null) {
        changes++;
        changes_html += "<li>Address line 2 from '" + customer.customeraddress2 + "' to '" + address2 + "'</li>" ;
    }

    var postcode = $("#id-postcode").val();
    if(postcode && postcode !== customer.customerpostcode && customer.customerpostcode !== null) {
        changes++;
        changes_html += "<li>Postcode from '" + customer.customerpostcode + "' to '" + postcode +"'</li>" ;
    }

    // var contact = $("#id-customer-contact-name").val();
    // if(contact && contact !== customer.customercontact && customer.customercontact !== null) {
    //     changes++;
    //     changes_html += "<li>Contact name from '" + customer.customercontact + "' to '" + contact + "'</li>";
    // }
    var firstName = $("#id-customer-firstname").val() ;
    if(firstName && firstName !== customer.customerfirstname && customer.customerfirstname !== null) {
        changes++;
        changes_html += "<li>First name from '" + customer.customerfirstname + "' to '" + firstName + "'</li>";
    }

    var lastName = $("#id-customer-surname").val() ;
    if(lastName && lastName !== customer.customersurname && customer.customersurname !== null) {
        changes++;
        changes_html += "<li>Surname name from '" + customer.customersurname + "' to '" + lastName + "'</li>";
    }

    var email = $("#id-customer-email").val();
    if(email && email !== customer.customeremail && customer.customeremail !== null) {
        changes++ ;
        changes_html += "<li>Email from '" + customer.customeremail + "' to '" + email + "'</li>" ;
    }

    var phone = $("#id-customer-phone").val();
    if(phone && phone !== customer.customerphonenumber && customer.customerphonenumber !== null) {
        changes++ ;
        changes_html += "<li>Phone number from '" + customer.customerphonenumber + "' to '" + phone + "'</li>"
    }

    var mobile = $("#id-customer-mobile").val();
    if(mobile && mobile !== customer.customermobilenumber && customer.customermobilenumber !== null) {
        changes++ ;
...
function getCustomer(customer_no, _cb) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            $("#id-customer-firstname").val(res.data.customerfirstname) ;
            $("#id-customer-surname").val(res.data.customersurname) ;
